...
class Maze:
    ...
    def __init__(self, graph, totalY, totalX):
        self._graph = graph
    ...
    #def Report(self, pos, val, pathTaken):
        #y,x = pos[0],pos[1]
        #assert(self.Lookup(y,x) == val)
        #if ((self._lowKey <= val <= self._highKey) or (self._lowDoor <= val <= self._highDoor)):
            #if (not self._graph.vertex_exists(chr(val))):
                #self._graph.addvertex(pos, chr(val))
